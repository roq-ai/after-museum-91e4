...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'purchase.create': { roles: ['tenant-roles', 'tenant-owner-roles'], key: 'purchaseId', tenantPath: [], userPath: [] },
  'product.create': { roles: ['tenant-roles'], key: 'productId', tenantPath: [], userPath: [] },
  'product.update': { roles: ['tenant-roles'], key: 'productId', tenantPath: [], userPath: [] },
};

const ownerRoles: string[] = ['owner'];
const customerRoles: string[] = ['visitor'];
const tenantRoles: string[] = ['owner', 'curator'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.museum.findFirst({
