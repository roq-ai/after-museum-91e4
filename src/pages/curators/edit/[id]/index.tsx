...
import { getCuratorById, updateCuratorById } from 'apiSdk/curators';
...
function CuratorEditPage() {
  ...
  const handleSubmit = async (values: CuratorInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateCuratorById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<CuratorInterface>({
    initialValues: data,
    validationSchema: curatorValidationSchema,
