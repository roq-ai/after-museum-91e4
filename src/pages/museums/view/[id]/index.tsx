...
import { getMuseumById } from 'apiSdk/museums';
import { MuseumInterface } from 'interfaces/museum';
import { CuratorListPage } from 'pages/curators';
import { ExhibitListPage } from 'pages/exhibits';
import { OwnerListPage } from 'pages/owners';
import { VisitorListPage } from 'pages/visitors';

function MuseumViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<MuseumInterface>(
    () => (id ? `/museums/${id}` : null),
    () =>
      getMuseumById(id, {
        relations: ['user'],
            ...
            <FormWrapper wrapperProps={{ gap: 3, p: 4 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('museum', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/museums/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              <EntityImage entity="museum" entityId={id} />
              <List
                w="100%"
                css={{
                  '> li:not(:last-child)': {
