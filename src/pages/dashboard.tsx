...
import { getVisitors } from 'apiSdk/visitors';
...
import { VisitorListPage as ShowCaseList } from 'pages/visitors';
...
function DashboardPage() {
  ...
  const visitorStat = useSWR<PaginatedInterface<VisitorInterface>>(
    () => `/visitors`,
    () => getVisitors({ limit: 1, offset: 0 }),
  );

  const exhibitStat = useSWR<PaginatedInterface<ExhibitInterface>>(
    () => `/exhibits`,
    () => getExhibits({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Visitors',
      result: visitorStat?.data?.totalCount,
      isLoading: visitorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Exhibits',
      result: exhibitStat?.data?.totalCount,
      isLoading: exhibitStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`visitors`} passHref legacyBehavior>
                  <Button
