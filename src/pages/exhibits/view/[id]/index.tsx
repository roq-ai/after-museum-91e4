...
import { getExhibitById } from 'apiSdk/exhibits';
import { ExhibitInterface } from 'interfaces/exhibit';
import { VisitorListPage } from 'pages/visitors';

function ExhibitViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<ExhibitInterface>(
    () => (id ? `/exhibits/${id}` : null),
    () =>
      getExhibitById(id, {
        relations: ['museum'],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('exhibit', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/exhibits/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Origin" text={data?.origin} />
                ...
                {hasAccess('museum', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Museum"
                    text={
                      <Link as={NextLink} href={`/museums/view/${data?.museum?.id}`}>
                        {data?.museum?.name}
                      </Link>
                    }
                  />
