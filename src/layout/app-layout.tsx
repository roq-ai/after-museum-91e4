...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'curator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<MuseumInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/museums` : null),
    () => getMuseums({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Museums',
      path: isTenantUser ? `/museums/view/${data?.[0]?.id}` : '/museums',
      entity: 'museum',
      service: AccessServiceEnum.PROJECT,
      icon: FiHome,
    },
    { name: 'Curators', path: '/curators', entity: 'curator', service: AccessServiceEnum.PROJECT, icon: FiFeather },
    { name: 'Exhibits', path: '/exhibits', entity: 'exhibit', service: AccessServiceEnum.PROJECT, icon: FiImage },
    { name: 'Owners', path: '/owners', entity: 'owner', service: AccessServiceEnum.PROJECT, icon: FiKey },
    { name: 'Visitors', path: '/visitors', entity: 'visitor', service: AccessServiceEnum.PROJECT, icon: FiEye },
